...
class FileManager(private val context: Context) {
    ...
    suspend fun ensureDirectoriesExist() = withContext(Dispatchers.IO) {
        try {
            ...
            // Remove deprecated Midi and Styles subfolders completely per user request
            try {
                val oldMidi = File(baseDir, "Midi")
                if (oldMidi.exists()) oldMidi.deleteRecursively()
                val oldStyles = File(baseDir, "Styles")
                if (oldStyles.exists()) oldStyles.deleteRecursively()
            } catch (_: Exception) {}
            ...
            listOf(baseDir, soundfontsDir, loopsDir, drumPadDir, scenesDir, presetsDir, logsDir, recordingsDir).forEach { dir ->
                try {
                    ...
                    if (!marker.exists()) {
                        marker.writeText("SoundStage Audio Directory")
                    }
                } catch (_: Exception) {}
            }
            ...
            try {
                ...
                pathsToScan.add(File(baseDir, ".soundstage").absolutePath)
