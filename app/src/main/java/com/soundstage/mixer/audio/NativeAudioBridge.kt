...
object NativeAudioBridge {
    ...
    fun safeSetMasterVolume(volume01: Float) {
        if (isLibraryLoaded) {
            try {
    ...
    fun safeSetPadBrightness(brightness: Float) {
        if (isLibraryLoaded) {
            try {
                setPadBrightness(brightness)
            } catch (e: Throwable) {
                Log.e("NativeAudioBridge", "Error invoking setPadBrightness: ${e.message}")
            }
        }
    }
